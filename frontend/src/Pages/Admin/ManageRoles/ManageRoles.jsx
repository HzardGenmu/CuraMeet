...
const ManageRoles = () => {
    ...
    const fetchCombinedData = async () => {
      ...
      try {
        const [patientResponse, doctorResponse] = await Promise.all([
          patientService.search(''), // Ambil semua pasien
          doctorService.getAll()      // Ambil semua dokter
        ]);
        
        let combinedUsers = [];

        // Proses dan normalisasi data pasien
        ...
        // Proses dan normalisasi data dokter
        // Axios membungkus respons di dalam 'data'
        if (doctorResponse.data && doctorResponse.data.success && Array.isArray(doctorResponse.data.doctors)) {
            const mappedDoctors = doctorResponse.data.doctors.map(d => ({
            id: `doctor-${d.id}`,
            userId: d.user_id,
            name: d.full_name,
          ...
          combinedUsers.push(...mappedDoctors);
        }
  ...
  const handleRoleChange = async (userId, newRole) => {
    ...
    try {
      ...
      setSuccess(`Role untuk pengguna berhasil diubah menjadi ${capitalize(newRole)}.`);
    } catch (error) {
  ...
  if (loading) {
    return <div className="p-8 text-center text-gray-600">Memuat daftar pengguna...</div>;
  }
        ...
        <div className="overflow-x-auto border border-gray-200 rounded-lg bg-white shadow-md mb-8">
          <table className="w-full border-collapse min-w-[600px]">
            <thead className="sticky top-0 bg-gray-50 z-10">
              <tr>
                <th className="py-3 px-4 text-left text-sm font-semibold text-gray-600 border-b">Nama</th>
              ...
              {currentUsers.length > 0 ? (
                currentUsers.map(user => (
                  <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                    <td className="py-3 px-4 text-sm text-gray-800 border-b border-gray-100">{user.name}</td>
                ...
                <tr>
                   <td colSpan="3" className="text-center p-6 text-gray-600">Tidak ada data ditemukan.</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="flex justify-center items-center mt-6">
            ...
            {[...Array(totalPages)].map((_, index) => (
              <button
                ...
                className={`py-2 px-4 border rounded-lg mx-1 text-sm ${currentPage === index + 1 ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-100'} transition-all duration-200`}
              >
                {index + 1}
              </button>
            ))}
